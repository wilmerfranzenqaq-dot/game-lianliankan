...
public class SaveManager {
    ...
    public static SaveData loadGame(String filepath) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))){
            ...
            // 检测存档版本：V2 有新字段(catName, totalPairs)，旧版没有
            boolean isV2 = "V2".equals(firstLine);
            String username = isV2 ? reader.readLine() : firstLine;
            String catName;
            String mode;
            ...
            int totalPairs = isV2 ? Integer.parseInt(reader.readLine()) : 0;
            ...
            if (isV2) {
                ...
                // 旧存档没有 totalPairs，从棋盘数据中计算
                int pairs = 0;
                for (int i = 0; i < rows; i++) {
                    for (int j = 0; j < cols; j++) {
                        if (board[i][j].getIconIndex() > 0) pairs++;
                    }
                }
                gameBoard.totalPairs = pairs / 2;
            }
